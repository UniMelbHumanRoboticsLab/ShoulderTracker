...
void Fl_TimerSimple::draw() {
  ...
  if (type() == FL_VALUE_TIMER && delay>0.0) {
    ...
    tt = (int) ((d+0.05) / 60.0);
    dd = (int) (tt / 60.0);
    sprintf(str, "%02d:%02d:%02d", dd, (int)(tt - 60.0 * dd), (int)(d - 60.0 * tt));
...
void Fl_TimerSimple::stepcb(void* v) {
  ((Fl_TimerSimple*)v)->step();
}

void Fl_TimerSimple::step() {
  if (!on) return;
...
int Fl_TimerSimple::handle(int event) {
...
*/
Fl_TimerSimple::~Fl_TimerSimple() {
  Fl::remove_timeout(stepcb, this);
}

/**
  Creates a new Fl_TimerSimple widget using the given type, position,
...
*/
Fl_TimerSimple::Fl_TimerSimple(uchar t, int X, int Y, int W, int H, const char* l)
...
void Fl_TimerSimple::value(double d) {
  delay = total = d;
  on = (d > 0.0);
...
void Fl_TimerSimple::suspended(char d) {
  if (!d) {
    if (on) return;
    on = (delay > 0.0);
    fl_gettime(&(lastsec), &(lastusec));
